...
const uint32_t PREV_POSITION_INDEX  = 3;
...
public:
    ...
    std::array<gl::VboRef, 2>			mPositions, mVelocities, mConnections, mPrevPositions;
    std::array<gl::BufferTextureRef, 2>	mPositionBufTexs;
    std::array<gl::BufferTextureRef, 2>	mPrevPositionBufTexs;
...
//            }
    ...
    // create your two BufferTextures that correspond to your prev position buffers.
//    mPrevPositionBufTexs[0] = gl::BufferTexture::create( mPrevPositions[0], GL_RGBA32F );
//    mPrevPositionBufTexs[1] = gl::BufferTexture::create( mPrevPositions[1], GL_RGBA32F );
...
//        gl::ScopedTextureBind prevScopeTex( mPrevPositionBufTexs[mIterationIndex & 1]->getTarget(), mPrevPositionBufTexs[mIterationIndex & 1]->getId() );
...
void sample::mouseDown( MouseEvent event )
{
...
void sample::mouseDrag( MouseEvent event )
{
    if( event.isRightDown() )
        mCamUi.mouseDrag( event.getPos(), true, false, false );
    else
...
void sample::mouseUp( MouseEvent event )
{
    updateRayPosition( event.getPos(), false );
}

void sample::updateRayPosition( const ci::ivec2 &mousePos, bool useDistance )
{
