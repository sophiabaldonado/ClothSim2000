...
layout (location = 3) in vec4 prev_position_mass;	// PREVIOUS_POSITION_INDEX
...
void main(void)
{
    ...
    pp = calcRayIntersection( pp );
    
    float m = position_mass.w;     // m is the mass of our vertex
    float pm = prev_position_mass.w;     // m is the mass of our previous vertex
    ...
    for( int i = 0; i < 4; i++) {
        if( connection[i] != -1 ) {
            ...
            vec3 q = texelFetch(tex_position, connection[i]).xyz; // - 0.01f; add this to make wind!?
            vec3 d = q - p;
            float x = length(d);
    ...
    vec4 temp = tf_position_mass;
    
    // Write the outputs
    tf_prev_position_mass = temp;
