...
#undef GET_VERTEX
    ...
    mLineIndices->unmap();
#else
    int cells_x = POINTS_X - 1;
    int cells_y = POINTS_Y - 1;
    ...
    for (int y = 0; y < cells_y; y++)
    {
        ...
        {
#define GET_VERTEX(x, y) ((x) + ((y) * POINTS_X))
            *pIndices++ = GET_VERTEX(x, y);
            *pIndices++ = GET_VERTEX(x + 1, y);
            *pIndices++ = GET_VERTEX(x + 1, y + 1);
            
            *pIndices++ = GET_VERTEX(x, y);
            *pIndices++ = GET_VERTEX(x + 1, y + 1);
            *pIndices++ = GET_VERTEX(x, y + 1);
#undef GET_VERTEX
        }
    }
...
void ClothSimVR::setupGlsl()
{
    ...
    updateFormat.vertex(loadAsset("updateVR.vert"))
...
{
    static vec3 gravity = vec3(0.0, -0.2, 0.0);
    static float restLength = 0.1;
    static float dampingConst = 0.1;
